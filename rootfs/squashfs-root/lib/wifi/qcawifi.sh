...
find_qcawifi_phy() {
	...
	[ -n "$phy" -a -d "/sys/class/net/$phy" ] || {
		echo "phy for wifi device $device not found"
		return 1
	}
	[ -z "$macaddr" ] && {
...
enable_qcawifi() {
	...
	config_get_bool burst "$device" burst 1
	...
	config_get_bool adapt_cca_thres "$device" adapt_cca_thres
	[ -n "$adapt_cca_thres" ] && iwpriv "$phy" CCAThEna "${adapt_cca_thres}"
	...
	for vif in $vifs; do
		...
		#Previously the use of HT40-,HT40 +,and later have HT40, qca recommended with the previous
		[ "$hwmode" = "11NGHT40" -o "$hwmode" = "11NAHT40" -a $channel -ne 0 ] && {
			if [ "$channel" -gt 7 ];then
				[ "$hwmode" =  "11NGHT40" ] && hwmode=11NGHT40MINUS
				[ "$hwmode" =  "11NAHT40" ] && hwmode=11NAHT40MINUS 
			else
				[ "$hwmode" =  "11NGHT40" ] && hwmode=11NGHT40PLUS
				[ "$hwmode" =  "11NAHT40" ] && hwmode=11NAHT40PLUS
			fi
		}
		...
		config_get_bool shpreamble "$device" shpreamble 0
		...
		config_get wds "$device" wds_enable
		case "$wds" in
			1|on|enabled) wds=1;;
			*) wds=0;;
		esac
		...
		config_get bintval "$device" bintval 100
		...
		[ -n "$extprotspac" ] && iwpriv "$ifname" extprotspac "$extprotspac"
                ...
                if [ "$ONLY_5G" = "1" -o "$device" = "wifi1" ]; then
			iwpriv "$ifname" cwmenable 1
                else
			config_get_bool cwmenable "$device" cwmenable
			[ -n "$cwmenable" ] && iwpriv "$ifname" cwmenable "$cwmenable"
                fi


		#config_get_bool protmode "$vif" protmode 0
		config_get bgprotect "$device" bgprotect
		if [ $bgprotect -eq 2 ]; then
			protmode=0
		else
			protmode=2
		fi
	...
	for vif in $vifs; do
		...
		local max_txpower= txpower_value=
		...
		#support nawds
		config_get nawds_mode "$device" nawds_mode
		[ -n "$nawds_mode" ] && wlanconfig "$ifname" nawds mode "${nawds_mode}"

		config_get wdslist "$device" wdslist
		[ -n "$wdslist" ] && {
			wlanconfig "$ifname" nawds mode 0
			wlanconfig "$ifname" nawds mode 1
			for mac in $wdslist; do
				wlanconfig "$ifname" nawds add-repeater "$mac" 0
			done
		}
		...
		case "$mode" in
			...
			sta)
				local bssid 
				local defbssid="00:00:00:00:00:00"
				config_get bssid "$vif" bssid
				if [ "$bssid" != "$defbssid" ]; then
					if eval "type wpa_supplicant_setup_vif" 2>/dev/null >/dev/null; then
		...
		[ -n "$vlanid" ] && iwpriv "$ifname" vlanid $vlanid
	done
	...
	[ -z "$default_txpower" ] || iwconfig "$tx_ifname" txpower "${default_txpower%%.*}"
	max_txpower=`iwconfig "$tx_ifname" | grep Tx-Power= | awk -F "Tx-Power=" '{print $2}' | awk -F " " '{print $1}'`

	if [ "$tx_ifname" = "ath1" ];then 
		max_txpower=`expr $max_txpower - 1`
	fi
	
	if [ "$txpower" -gt 75 ];then
		txpower_value=$max_txpower
	elif [ "$txpower" -gt 50 ];then
		txpower_value=`expr $max_txpower - 2`
	elif [ "$txpower" -gt 25 ];then
		txpower_value=`expr $max_txpower - 4`
	elif [ "$txpower" -gt 12 ];then
		txpower_value=`expr $max_txpower - 6`
	elif [ "$txpower" -gt 6 ];then
		txpower_value=`expr $max_txpower - 9`
	elif [ "$txpower" -gt 3 ];then
		txpower_value=`expr $max_txpower - 12`
	elif [ "$txpower" -gt 0 ];then
		txpower_value=`expr $max_txpower - 15`
	else
		txpower_value=$max_txpower
	fi
	[ -z "$txpower_value" ] || iwconfig "$tx_ifname" txpower "${txpower_value%%.*}"

	[ $channel -gt 0 ] && iwconfig "$tx_ifname" channel "$channel"
...
enable_multiap() {
	...
	ifname=$(/usr/sbin/wlanconfig "$ifname" create wlandev "$phy" wlanmode "$mode" ${wlanaddr:+wlanaddr "$wlanaddr"} ${nosbeacon:+nosbeacon})
	[ $? -ne 0 ] && {
		echo "enable_qcawifi($phy): Failed to set up $mode vif $ifname" >&2
		return;
	}
...
post_qcawifi() {
	...
	case "${action}" in
		enable)
			...
			config_get_bool wps_vap_tie_dbdc qcawifi wps_vap_tie_dbdc 0
			config_get_bool wps_flag "$vif" wps_flag 0
...
detect_qcawifi() {
	...
	for dev in $(ls -d wifi* 2>&-); do
		...
		if [ "$DEFAULT_SSID" = "TOTOLINK_MAC6" ]; then
			ssid_d=TOTOLINK_$(cat /sys/class/net/${dev}/address | awk -F ":" '{print $4""$5""$6 }'| tr a-z A-Z)
		else
			ssid_d=$DEFAULT_SSID
		fi
		[ "$dev" = "wifi1" ] && ssid_d=${ssid_d}_5G
		...
		hardModel=$(uci -q get product.custom.hardModel)

		if [ "${hardModel}" = "IP04315" ];then
			defcountry=US
		fi
		...
		case "${hwcaps}" in
			*11bgn)
			if [ "${hardModel}" = "IP04315" ];then
				mode_11=ng;mode_ht=HT40
			else
				mode_11=ng;mode_ht=HT20
			fi
			;;
			*11abgn) mode_11=ng;mode_ht=HT40;;
			*11an) mode_11=na;mode_ht=HT40;;
			*11an/ac) mode_11=ac;mode_ht=HT80;;
			*11abgn/ac) mode_11=ac;mode_ht=HT80;;
		esac
...
config wifi-device  wifi$devidx
	...
	option cwmenable '0'
	option adapt_cca_thres '1'
	option BssidNum 1
	option disabled 0
	option country ${defcountry}
	option bintval 100
	...
	option macfilter 0
	option maclist ''
	option mac_cmnt_list ''
	option wds_enable 0
	option wdslist ''
	option wds_cmnt_list ''
...
config wifi-iface 'apcli$devidx'
	option ifname 'apcli$devidx'
	option device 'wifi$devidx'
	option network	'lan'
	option mode 'sta'
	option disabled '1'
	option ssid 'TOTOLINK RPT_${BaudSSID}'
...
config wifi-iface 'ath${devidx}1'
	option ifname 'ath${devidx}1'
	option device 'wifi$devidx'
	option network	'lan'
	option mode 'ap'
	option disabled '1'
...
config wifi-iface 'ath${devidx}2'
	option ifname 'ath${devidx}2'
	option device 'wifi$devidx'
	option network	'lan'
	option mode 'ap'
	option disabled '1'
...
config wifi-iface 'ath${devidx}3'
	option ifname 'ath${devidx}3'
	option device 'wifi$devidx'
	option network	'lan'
	option mode 'ap'
	option disabled '1'
...
config wifi-iface 'ath${devidx}4'
	option ifname 'ath${devidx}4'
	option device 'wifi$devidx'
	option network	'lan'
	option mode 'ap'
	option disabled '1'
...
config wifi-iface 'wds$devidx'
	option ifname 'wds$devidx'
	option device 'wifi$devidx'
	option network	'lan'
	option mode 'sta'
	option disabled '1'
	option ssid 'TOTOLINK WDS'
	option bssid '00:00:00:00:00:00'
	option encryption 'none'
	option keys ''
	option key ''
