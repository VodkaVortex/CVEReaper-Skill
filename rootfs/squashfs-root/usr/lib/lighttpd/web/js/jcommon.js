...
function do_count_down4(){
	supplyValue('show_sec',wtime);
	if(wtime == 0) {top.location.reload();}
	if(wtime > 0) {wtime--;setTimeout('do_count_down4()',1000);}
}
...
function switchLanguage(val){
	...
	if(val == 'auto'){
		postVar['autoFlag'] = "1";
		postVar['langType'] = autoLangFun('get');
	}else{
		postVar['autoFlag'] = "0";
...
/**
 * 当输入完IP地址后，根据当前IP地址自动生成掩码。
 * ip,mask分别是IP和MASK的id 
 */
function autoChangeMask(ip,mask){
	var ip1="#"+ip+"1";
	var ip2="#"+ip+"2";
	var ip3="#"+ip+"3";
	var ip4="#"+ip+"4";
	...
	if(($(ip1).val()!="")&&($(ip2).val()!="")&&($(ip3).val()!="")&&($(ip4).val()!="")){
		var ipadd1=parseInt($(ip1).val());
		if((ipadd1==127)||(ipadd1==0)){
				alert(JS_msg60);
				return false;
			}
		
		if((ipadd1>0)&&(ipadd1<=127)){//A类地址
			$(mask1).val("255");
			$(mask2).val("0");
			$(mask3).val("0");
			$(mask4).val("0");
		}else if((ipadd1>127)&&(ipadd1<192)){//B类地址
			$(mask1).val("255");
			$(mask2).val("255");
			$(mask3).val("0");
			$(mask4).val("0");
		}else if((ipadd1>=192)&&(ipadd1<=223)){//C类地址
			$(mask1).val("255");
			$(mask2).val("255");
			$(mask3).val("255");
			$(mask4).val("0");
...
/**
 * 当按键弹起的时候监听当前输入框的字符长度，如果长度==3就调到下一个输入框
 * （同时排除数字以外的所有输入。否则当字符长度为3的时候，随便按一个都会跳转到下一个。）
 */
function ipValiOnKeyUp(e,n,i)
{
	var co = e.keyCode;
	var inputs = document.getElementsByName(n);
	if(co==9 || co==37 || co==39 || co==110 || co==190)
	 return false;
	// 除了输入数字，其他的都不进行字符长度的判断
...
function ipVali(e, n, i){
	...
	//如果有选中，按.失效
	if (getSelectedText(inputs[i]) != "" && (co==110 || co==190) )
		return false;
...
function showLanguageLabel(){
	...
	document.write("<div id=\"languageDiv\" style=\"display:none\">\
	<table width=172 border=0 cellpadding=3 cellspacing=0>\
	<tr><td colspan=2 height=13></td></tr>\
	<tr><td class=\"languageTitle\" onclick=\"switchLanguage('en')\">"+MM_english+"</a></td>\
	<td><img id=\"language_en\" src=\"../style/language_check.gif\" border=0></td></tr>\
	<tr id=\"div_languageCn\" style=\"display:none\"><td class=\"languageTitle\" onclick=\"switchLanguage('cn')\">"+MM_chinese_simplified+"</a></td>\
	<td><img id=\"language_cn\" src=\"../style/language_no_check.gif\" border=0></td></tr>\
	<tr id=\"div_languageCnt\" style=\"display:none\"><td class=\"languageTitle\" onclick=\"switchLanguage('cnt')\">"+MM_chinese_traditional+"</a></td>\
...
function showLanguageOption(){
	data = _globalCfg_;
	var id = $('#languageOption');
	var tempLang = data["MultiLangBuilt"].split(";");
	
	// if(tempLang.length == 1)
	// 	return;

	id.append('<option value="auto">'+MM_autolang+'</option>');
	var msg;
	for(var i=0;i<tempLang.length;i++){
		if(tempLang[i] == 'en'){
			msg = MM_english;
		}else if(tempLang[i] == 'cn'){
			msg = MM_chinese_simplified;
		}else if(tempLang[i] == 'cnt'){
			msg = MM_chinese_traditional;
		}else if(tempLang[i] == 'vn'){
			msg = MM_vn;
		}
		id.append('<option value="'+tempLang[i]+'">'+msg+'</option>');
	}
	if(data['autoFlag'] == "1")
		id.val('auto');
	else
		id.val(data['LanguageType']);
	id.on('change',function(){
		switchLanguage(id.val());
	});
}
...
function sort_diy(arr) {
	var p2=";m1t7k|";
	for (var i = 0; i < arr.length; i++) {
		arr[i] = arr[i].split(p2);
	}
