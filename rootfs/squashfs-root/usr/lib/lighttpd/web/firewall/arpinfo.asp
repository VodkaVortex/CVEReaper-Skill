...
function selectArpTbl(ip, mac){
	...
	if (flag==1){
		...
		window.opener.document.formFilterAdd.sip.value = ip.split(".")[3];
	}else if (flag==3){
		window.opener.document.formPortFwdAdd.ip.value = ip.split(".")[3];
	}else if (flag==4) {
		...
		window.opener.document.formStaticDhcpAdd.ip.value = ip.split(".")[3];
	}else if (flag==6){
